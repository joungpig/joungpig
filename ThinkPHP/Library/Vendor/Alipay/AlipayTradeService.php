...
class AlipayTradeService {
 	...
 	*/
	function pagePay($builder,$return_url,$notify_url,$request,$Aop) {
		...
		// 首先调用支付api
		$response = $this->aopclientRequestExecute ($request,true,$Aop);
	 ...
	 * @return $response 支付宝返回的信息
 	*/
	function  aopclientRequestExecute($request,$ispage=false,$Aop,$SignData) {
		...
		{
			$result = $aop->Execute($request,$SignData);
		}
	 ...
	 * @return $response 支付宝返回的信息
 	*/
	function Query($builder,$request,$Aop,$SignData){
		$biz_content=$builder->getBizContent();
		//打印业务参数
		$request->setBizContent ( $biz_content );

		$response = $this->aopclientRequestExecute ($request,false,$Aop,$SignData);
